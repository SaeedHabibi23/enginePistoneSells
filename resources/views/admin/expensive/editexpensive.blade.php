...
<form method="post" action="../StoreUpdateexpensive">
    @csrf
    <input type="hidden" name="expensive_id" value="{{$expensive->expensive_id}}">
    <div>
        <label for="name"> قیمت </label>
        <input type="text" id="number" value="{{$expensive->price}}"   name="price" class="form-control">
    </div>
 تاریخ <input type="date" value="{{$expensive->date}}" name="date" class="form-control"><br>
   توضیحات <input type="text" value="{{$expensive->description}}" name ="description" class="form-control"><br>
